...
目前網站使用最新 5,000 筆明細產生匿名摘要，呈現 COD、SS、pH 的樣本數、中位數與 P90。這是依官方預覽排序取得的最新批次，不是 191 萬筆全資料母體，也不是化工業專屬樣本，因此不直接拿來訓練逐時預報模型。網站同時提供依申報截止日聚合的 COD、SS、pH 中位數趨勢圖，以及化工業中央基準情境比較；兩者都只供資料探索，不代表個別事業合規判定。進水預報頁另使用保留測試集殘差顯示 80% 經驗預測區間與超過參考限值的研究機率；這不是經臺灣化工廠校準的法規超標機率。
