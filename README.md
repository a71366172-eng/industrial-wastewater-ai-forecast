...
欄位包含進出水 pH、導電度、SS、COD、流量、投藥、曝氣、污泥濃度、時間與品質旗標。正式提前預報需要逐時資料及正確的水力停留時間；環境部 EMS_S_03 申報資料可作為臺灣濃度分布與外部檢查資料，但時間粒度不足以單獨支撐逐時預報。依自動監測連線規定，pH、導電度、水量與水溫至少每 5 分鐘傳輸，COD、SS 等至少每小時傳輸；專案正式模型因此採每小時時間網格與未來 1 小時預測目標。目前已完成 1／3／6 小時未來視窗標籤、滯後特徵、時間切分、Logistic 機率基線與 Brier score 評估管線；達到 2,160 筆逐時資料及每個時距 30 次超標事件後才會產生正式研究模型。
...
目前網站使用最新 5,000 筆明細產生匿名摘要，呈現 COD、SS、pH 的樣本數、中位數與 P90。這是依官方預覽排序取得的最新批次，不是 191 萬筆全資料母體，也不是化工業專屬樣本，因此不直接拿來訓練逐時預報模型。網站同時提供依申報截止日聚合的 COD、SS、pH 中位數趨勢圖，以及化工業中央基準情境比較；兩者都只供資料探索，不代表個別事業合規判定。進水預報頁另使用保留測試集殘差顯示 80% 經驗預測區間與超過參考限值的研究機率；這不是經臺灣化工廠校準的法規超標機率。導覽列的「趨勢預測」頁可查看臺灣申報期間趨勢，以及 UCI 保留測試集的實際值／預測值雙折線。
